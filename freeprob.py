...
def compute_free_prob(probs, input_sequence):
    """
    ...
    Input: probs: dictionary from tuples of symbols to probabilities
    ...
    if len(free_sequence) == 1:
        return multiplier*probs[free_sequence[0]]

    poly = [(s, -probs[s]) for s in free_sequence]
    ...
    for expression in expressions:
        ...
        for e in expression:
            if type(e) == float:
                ...
                unpacked += list(e)
        total -= compute_free_prob(probs, unpacked)
